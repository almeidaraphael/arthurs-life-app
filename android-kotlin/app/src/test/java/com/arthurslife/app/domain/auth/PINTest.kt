...
class PINTest {
    ...
    inner class SecurityRequirements {
        ...
        fun shouldResistTimingAttacks() {
            ...
            assertTrue(
                times.all { it > avgTime * 0.5 && it < avgTime * 1.5 },
                "Verification times should be consistent to resist timing attacks",
            )
        }
    ...
    inner class EdgeCasesAndErrorConditions {
        ...
        fun shouldHandleConcurrentVerification() {
            ...
            // Test concurrent verification (basic thread safety)
            val results = (1..100).toList().parallelStream().map {
                pin.verify("1234")
            }.toList()
    ...
    inner class BusinessRulesAndIntegration {
        ...
        fun shouldSupportPinEqualityComparison() {
            ...
            assertFalse(pin3.verify("1234"), "PIN3 should not verify different value")

            // But hashes should be different due to salting
            assertNotEquals(
                pin1.getHash(),
                pin2.getHash(),
                "Equal PINs should have different hashes",
            )
        }
    }
}
