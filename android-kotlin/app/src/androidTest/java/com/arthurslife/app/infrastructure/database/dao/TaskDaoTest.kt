...
class TaskDaoTest {
    ...
    inner class CreateOperations {
        ...
        fun shouldInsertMultipleTasks() = runTest {
            // Given
            val tasks = listOf(
                EntityTestFactory.createTaskEntity(
                    id = "task-1",
                    title = "Brush teeth",
                    assignedToUserId = "user-1",
                    customIconName = "default_icon",
    ...
    inner class DeleteOperations {
        ...
        fun shouldDeleteOnlySpecifiedTask() = runTest {
            // Given
            val tasks = listOf(
                EntityTestFactory.createTaskEntity(id = "task-1"),
                EntityTestFactory.createTaskEntity(id = "task-2"),
                EntityTestFactory.createTaskEntity(id = "task-3"),
            )
            tasks.forEach { taskDao.insert(it) }
    ...
    inner class DomainConversion {
        ...
        fun shouldConvertFromDomainModel() = runTest {
            // Given
            val domainTask = com.arthurslife.app.domain.TestDataFactory.createTask(
                id = "domain-task",
                title = "Domain Task",
                category = com.arthurslife.app.domain.task.TaskCategory.HOMEWORK,
                assignedToUserId = "user-1",
                isCompleted = true,
            )
